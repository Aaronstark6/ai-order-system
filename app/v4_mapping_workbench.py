...
def _save_structured(items):
    data = _read_json(STRUCTURED_RULE_PATH, {"version": "V4-Rebuild.11", "mappings": []})
    ...
    data["version"] = data.get("version") or "V4-Rebuild.11"
...
def _save_tables(items):
    data = _read_json(TABLE_RULE_PATH, {"version": "V4-Rebuild.11", "tables": []})
    ...
    for item in items:
        ...
        for column in columns:
            ...
            normalized_columns.append(
                {
                    "label": label,
                    "field": str(column.get("field") or _field_key(label)),
        ...
        table = {
            ...
            "start_cell": str(item.get("start_cell") or _start_cell(item)),
...
def _save_blocks(items):
    data = _read_json(BLOCK_RULE_PATH, {"version": "V4-Rebuild.11", "blocks": []})
...
def save_selected_mappings(payload):
    ...
    if not structured and not tables and not blocks:
        return {
            "success": False,
            "error": "没有选择任何映射",
        }
    ...
    result = {
        "structured_saved": _save_structured(structured),
        "tables_saved": _save_tables(tables),
        "blocks_saved": _save_blocks(blocks),
    }
    return {
        "success": True,
        "message": "映射规则保存成功",
