...
from app.v4_ai_template_parser import analyze_template
...
def execute_ai_template_to_excel(example_name: str, template_path: str) -> dict:
    warnings = []
    ...
    try:
        ...
        logger.info(
            "V4 AI template export started: example_name=%s template_path=%s",
            example_name,
            template_path,
        )
        ...
        field_library = {
            "fields": load_fields(),
            "description_fields": list(description_fields.keys()) if isinstance(description_fields, dict) else [],
        }
        parser_result = analyze_template(template_path, field_library)
        if not parser_result.get("success"):
            return {
                "success": False,
                "error": parser_result.get("error", "V4 AI template parser failed"),
                "warnings": warnings + parser_result.get("warnings", []),
            }
        warnings.extend(parser_result.get("warnings", []))
        ...
        if not preview_result.get("success"):
            return {
                "success": False,
                "error": preview_result.get("error", "V4 Excel rule preview failed"),
                "warnings": warnings + preview_result.get("warnings", []),
            }
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_example_name = _safe_output_filename_part(example_name)
        safe_template_name = _safe_output_filename_part(Path(template_path).stem)
        filename = f"{safe_example_name}_{safe_template_name}_ai_template_{timestamp}.xlsx"
        output_path = output_dir / filename
        ...
        logger.info(
            "V4 AI template export succeeded: output_path=%s operations_written=%s warnings=%s",
            output_path,
            executor_result.get("operations_written", 0),
            len(warnings),
        )
        ...
        logger.exception(
            "V4 AI template export failed: example_name=%s template_path=%s",
            example_name,
            template_path,
        )
        return {
